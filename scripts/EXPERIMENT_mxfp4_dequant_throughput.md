...
**This experiment confirms or replaces that ~6,300 projection with a measured
number.**
...
| arm | tps (steady avg) | vs old | vs bf16 |
|---|---|---|---|
| old mxfp4 | (measure; expect ~4,970) | 1.0× | — |
| **new mxfp4** | **(measure; projection ~6,300)** | **(ratio)** | — |
| bf16 (opt.) | (~7,740) | — | 1.0× |

Plus: confirm step-0 loss matches between arms (sanity — must be bit-identical),
and note GPU mem/rank. If the measured new-vs-old ratio at 4096 packed differs
materially from ~1.27×, update the projection in the `perf(mxfp4)` commit
message / PR.
